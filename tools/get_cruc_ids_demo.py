...
if resp.ok:
    ...
    acc_resp = requests.get(acc_url, headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json", "accNum": "0"})
    if not acc_resp.ok:
        acc_resp = requests.get(acc_url, headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json", "accNum": "1"})
        if not acc_resp.ok:
            acc_resp = requests.get(acc_url, headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json", "accNum": "2"})
    ...
    if acc_resp.ok:
        ...
        if accounts:
            ...
            if inst_resp.ok:
                ...
                targets = ["US30", "NAS100", "SPX", "EURUSD", "GBPUSD", "XAUUSD", "XAGUSD", "CADJPY", "NZDJPY", "USDHKD", "USDCNH", "BRENT", "WTI"]
